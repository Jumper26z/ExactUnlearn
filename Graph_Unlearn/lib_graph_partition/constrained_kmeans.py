...
class ConstrainedKmeans:
    ...
    def initialization(self):#随机从所有节点里挑 num_clusters 个点，作为初始聚类中心
    ...
    def clustering(self):
        ...
        for i in range(self.max_iteration):
            ...
            delta = float(self._centroid_delta(centroid, self.centroid))  #2025/5/18解决偶尔不触发判断条件

            km_delta.append(delta)
            centroid = copy.deepcopy(self.centroid)

            if delta <= self.terminate_delta:
                self.logger.info(f"Early stopping at iteration {i} with delta={delta}")
                break
